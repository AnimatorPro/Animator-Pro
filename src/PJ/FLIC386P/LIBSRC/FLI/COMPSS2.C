...
#ifdef OLDSLOW
static int til_next4_same(SHORT *a, const int ct)

/* returns word count till next 4 words in a row that are the same */
{
int numsame;
SHORT lastval;
int spos = 0;
...
static int lwcont(SHORT *s1, SHORT *s2, int count) 
/* keep going until 2 short matches */
{
SHORT *maxs1;
int diff = 0;
...
static SHORT ss2_line(SHORT *s1, SHORT *s2, SHORT **pcbuf, 
					   int count, int skipct) 
{
SHORT opcount = 0;
BYTE *packet;
...
SHORT *ops;
	...
	/* Now compress it a packet at a time... */
	while (count > 0)
	{
		if (skipct == count)	/* same until end */
			break;
		count -= skipct;
		s1 += skipct;
		while (skipct > 127)
		{
		static BYTE pakdat[2] = { (127*2), 1 };
		...
		*packet++ = (skipct<<1);
		...
		else
		{
			if (copyct > 127)
				copyct = 127;
			samect = pj_fsame(s2, copyct);
			if (samect == copyct || samect > INERT)
			{
				*cbuf++ = *s2;
				*packet = -samect;
				s1 += samect;
			...
			{
				copyct = til_next4_same(s2, copyct);
				pj_copy_words(s2, cbuf, copyct);
				*packet = copyct;
				s1 += copyct;
				s2 += copyct;
				cbuf += copyct;
		...
		skipct = pj_fcompare(s1, s2, count);
	}
	*pcbuf = cbuf;
	return(*ops = opcount);
}
