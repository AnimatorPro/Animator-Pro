/* raster4.c -
 * 	raster portion of a driver that divides VRAM 640x400 screen into
...
static void vq_ss2_cleanup(VramRast *v, void *ucbuf, 
...
static void vq_unss2_rect(VramRast *r,void *ucbuf, LONG pixsize,
...
if (width&1)
	vq_ss2_cleanup(r,ucbuf, x, y, width);
}


static void vq_mask1_blit(UBYTE *mbytes, Coor mbpr, Coor mx, Coor my,
...
static void vq_xor_rect(VramRast *r, Pixel color, 
...
while (--height>=0)
	{
	_vram_x_hline(r, color, x, y++, width);
	}
}

static Pixel vq_get_dot(VramRast *r, Coor x, Coor y)
{
return _vram_get_dot(r, x+r->hw.vm.xoff, y+r->hw.vm.yoff);
}

static void vq_put_dot(VramRast *r,Pixel color, Coor x, Coor y)
{
_vram_put_dot(r, color, x+r->hw.vm.xoff, y+r->hw.vm.yoff);
}

static void vq_set_hline(VramRast *r,Pixel color, Coor x, Coor y, Ucoor width)
{
_vram_d_hline(r,color, x+r->hw.vm.xoff, y+r->hw.vm.yoff, width);
}

static void vq_get_hseg(VramRast *r, void *pixbuf,  Ucoor x, Ucoor y, Ucoor w)
{
_vram_get_hseg(r,pixbuf,x+r->hw.vm.xoff,y+r->hw.vm.yoff,w);
}

static void vq_put_hseg(VramRast *r, void *pixbuf,  Ucoor x, Ucoor y, Ucoor w)
{
_vram_put_hseg(r,pixbuf,x+r->hw.vm.xoff,y+r->hw.vm.yoff,w);
}

extern void vq_close_raster(VramRast *r);


Rastlib vram_raster_library;
...
static got_lib = 0;
	...
	if (!got_lib)
	{
		vram_raster_library.close_raster = vq_close_raster;
		vram_raster_library.put_dot = vq_put_dot;
		vram_raster_library.get_dot = vq_get_dot;
		vram_raster_library.set_hline = vq_set_hline;
		vram_raster_library.put_hseg = vq_put_hseg;
		vram_raster_library.get_hseg = vq_get_hseg;
		vram_raster_library.mask1blit = vq_mask1_blit;
		vram_raster_library.unss2_rect = vq_unss2_rect;
		vram_raster_library.xor_rect = vq_xor_rect;
