...
line_loop:
	lodsw
	or  ax,ax 
           jz  next_line               ; Go to next line for op word of 0
	...
	; put dot in al at x + width - 1 here
           ...
           WAITQ     7
           pop       AX
	mov dx,FRGD_COLOR           ; Set Color
	and ax,0ffh
	out dx,ax
	Set_fore                    ; Set normal mix
	mov eax,[ebp].uss_y
	mov dx,CUR_Y_POS            ; Set Y
	out dx,ax
	mov eax,[ebp].uss_x
	mov dx,CUR_X_POS            ; Set X to x+width-1
           add       EAX, [ebp].uss_width
           dec       EAX
           out       DX, AX

           xor       AX, AX            ; One pixel (1 - 1 = 0)
