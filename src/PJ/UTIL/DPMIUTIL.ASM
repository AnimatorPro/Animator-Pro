...
_TEXT	segment
	...
	public	pj_dpmi_present
	public	pj_dpmi_inquire_version
	public	pj_dpmi_inquire_memory


;*****************************************************************************
;* Boolean pj_dpmi_present
;*  returns TRUE if DPMI is present
;*****************************************************************************
pj_dpmi_present proc
PHAR_CONFIG_SIZE equ 512
PHAR_CONFIG_DPMI_FLAG equ 0D8h
	Entry
	Save	ebx,esi,edi,es

	; Make space on the stack for configuration buffer
	sub		esp,PHAR_CONFIG_SIZE

	; Call Phar Lap "Get configuration information"
	mov		ebx,esp
	mov		ecx,esp
	mov		eax,2526h
	int		21h

	; And return DPMI flag in eax
	mov		eax,[ebx+PHAR_CONFIG_DPMI_FLAG]

	; Clean up and leave.
	add		esp,PHAR_CONFIG_SIZE
	Restore ebx,esi,edi,es
	Exit
pj_dpmi_present endp
