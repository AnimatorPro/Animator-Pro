...
extern char recordall, clickonly, realtimemac; /* how much input to record... */
...
#define CBUF_SIZE (64000+3*COLORS+256+16+sizeof(struct video_form) )
...
extern char tflxname[], new_tflx_name[], tmacro_name[], alt_name[], cel_name[],
	...
	crestore_name[], another_name[],
	opoly_name[], default_name[], ppoly_name[], bscreen_name[];
...
struct vsettings 
	...
	WORD ado_turn;
	WORD ado_szmouse;
	struct ado_setting first3, move3;
	WORD ado_first;
	WORD ado_reverse;
	WORD ado_complete;
	WORD ado_source;
	...
	unsigned char menu_black[3], menu_grey[3], menu_light[3], menu_bright[3],
		menu_red[3], menu_extra[3];
	WORD sep_box;
	WORD marks[4];
	WORD starttr[4];
	WORD stoptr[4];
