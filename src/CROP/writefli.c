...
if (type == FLI_BRUN)
	c = full_cmap((char *)(chunk+1), this_cmap);
else
	c = fccomp(last_cmap, this_cmap, chunk+1, COLORS);
...
switch (type)
	{
	case FLI_LC:
		c = lccomp(last_screen, this_screen, chunk+1, 320, 200);
		break;
	case FLI_BRUN:
		c = brun(this_screen, this_screen, chunk+1, 320,200);
		break;
	}
if (c == NULL)
	{
...
{
if (!fli_save_frame(wfli_name,wfli_fd,cbuf,(char *)last_screen, 
	(char *)last_cmap, (char *)first_screen, (char *)first_cmap, FLI_LC) )
	{
	truncated(wfli_name);
	return(0);
	}
