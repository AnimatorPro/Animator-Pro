...
static void load_overlay(int how)
{
	...
	title = pj_get_filename(stack_string(overl_msgs[how], sbuf), ".FLC;.CEL;.FLI", ok_str,
							cpath.path, NULL, false, &cpath.scroller_top, cpath.wildcard);
		...
		switch (how) {
			...
			case 2: /* crossfade */
				...
				if ((err = transpblit(loadcel, 0, false,
									  (i * (long)100 + flif.hdr.frame_count / 2) / fcount)) <
					Success) {
					goto error;
				}
				break;
