...
static void changetocel(UBYTE* oldline, UBYTE* newline, Rcel* cel, SHORT y, Pixel tcolor);
...
{
	zoom_undo_rect(fc->xf.ommax.x, fc->xf.ommax.y, fc->xf.ommax.width, fc->xf.ommax.height);
}
...
void marqi_flicel(Flicel* fc, int dotmod, Pixel* save_buf)
{
	...
	if ((mh.dotbuf = save_buf) == NULL)
		...
		msome_vector(thecel->xf.bpoly, 4, savedraw_marqidot, &mh, 0, sizeof(Short_xy));
	}
}
 ...
 * under tempname */
static Errcode clip_from_fli(char* tempname, char* fliname, Flicel** pfcel, Rectangle* rect)
{
	Errcode err;
	Rcel* clip;
...
static Errcode clip1_celframe(void* clipceldat, int ix, int it, int scale, Autoarg* aa)
{
	...
	if (!aa->in_preview) {
		...
		if (aa->cur_frame == (aa->frames_in_seq - 1)) /* last frame, ringit */
		{
			if (aa->cur_frame != 0) {
				...
				if ((err = pj_i_read_uncomp1(cd->fliname, &fc->flif, fc->rc, cbuf, 0)) < 0) {
					goto error;
				}
...
/* Have user define a polygon shape then grab a cel from within shape */
...
static Errcode fcel_putline(void* plinedat, Pixel* line, Coor x, Coor y, Ucoor width)
{
