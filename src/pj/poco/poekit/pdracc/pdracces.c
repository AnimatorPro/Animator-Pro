...
/*****************************************************************************
 * PDRACCES.C - Provide extended access to picture PDRs from poco.
 *
 * 09/13/92 Ian
 *			Added logic to fill in the ainfo structure with data representing
 *			the target screen before calling pdr->open_ifile().  (It used to
 *			just zero-out the structure).  This is required to support
 *			variable-rez PDRs (which seem to exist only in theory anyway.)
 *			Also, in pic_load(), we now check for the PDR having reported
 *			ainfo.width or ainfo.height as zero, and return Err_format if so.
 *			This prevents Bad Cookie and other errors that can happen when
 *			trying to use a zero-dimensioned virtual screen.
 ****************************************************************************/
...
static void get_screen_ainfo(Rcel *screen, Anim_info *spec)
/*****************************************************************************
 * loads an anim_info to represent a screen
 ****************************************************************************/
{
	if (screen == NULL) {			/* might be NULL on a call to do an 	*/
		screen = GetPicScreen();	/* open_file just to query pic sizes.	*/
	}
...
static Errcode open_ifile(char *path, Rcel *screen)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	get_screen_ainfo(screen, &ainfo);	/* preload ainfo for var-rez pdrs */
 ...
 ****************************************************************************/
{
	...
	poeGetResourceDir(ptr2ppt(pdrpath,sizeof(pdrpath)));
	if (builtin_err)
		return builtin_err;
 ...
 ****************************************************************************/
{
	...
	if (Success > (err = open_ifile(path.pt,NULL))) /* open fills in ainfo */
		return err;
 ...
 ****************************************************************************/
{
	 ...
	 * the main point of this complicated retry shit is to allow a poco
...
RETRY:
	...
	if (Success > (err = open_ifile(path.pt, screen.pt))) {
		if (allow_retry) {
			pdr_unload();
			goto RETRY;
		} else
	...
	if (ainfo.width == 0 || ainfo.height == 0) {
		err = Err_format; /* rude PDR reported success for bogus picture data */
	} else {
