;*****************************************************************************
;* WCINT10.ASM - Issue int10/getinfo to vesa bios (Watcom/DOS4GL/DPMI).
;*
...
;*
;*  06/20/94 -  Ian
;*              Updated the flat->segmented conversion to match the new
;*              way we allocate dos-memory buffers.  Our caller now passes
;*              in the DOS segment/paragraph number in DI.
;*****************************************************************************
...
;*       di = DOS segment/paragraph number.
;*  Output:
...
_pj_vesa_int10 proc near
        ...
        mov    [eax].rr_ds,di           ; dos segment/paragraph number is
        mov    [eax].rr_es,di           ; passed in by the caller in DI.
        xor    edi,edi                  ; dos offset is always zero.
        mov    [eax].rr_edi,edi
 
        mov    edi,eax                 ; move realparms pointer to parm reg.
