...
static bool is_rect_and_menu_rect_same(const Rectangle *r, const Menuhdr *m)
{
	 ...
	 */
	return (r->x == m->x) && (r->y == m->y) && (r->width == m->width) && (r->height == m->height);
}
...
int button_keyhit(Menuhdr *mh, Button *mbs, VFUNC prehit)
{
	...
	{
		if ((UBYTE)icb.inkey == '\t' && (b = mh->group->tabnext) != NULL && b->root == mh->mw) {
			goto got_keyhit;
...
got_keyhit:
		...
		mw = mh->mw;
		if (NULL != (b = hit_button(mbs, icb.cx - mw->w.x, icb.cy - mw->w.y))) {
			bflags = b->flags;
			if (JSTHIT(MBPEN)) {
				...
				if (((bflags & (MB_DISABLED | MB_DISABOPT)) != (MB_DISABLED | MB_DISABOPT)) &&
					!(bflags & MB_OPTISDATA) && ((doit = b->optme) != NULL)) {
					check_reqpos_cancel(mh);
					ret = 1; /* mouse hit */
					goto do_hit;
				}
