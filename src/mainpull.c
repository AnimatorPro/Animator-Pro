...
/* mainpull.c - The data structures for the top level drop-down menus up
   along the top.  Also a routine to disable menu options we can't deal
   with yet. */
...
/* set disable flags and goes to do the pull */
{
	static SHORT cel_pulltab[] = {
		CEL_REL_PUL, CEL_TUR_PUL, CEL_STR_PUL, CEL_MOV_PUL, CEL_PAS_PUL,
	};

	static SHORT anim_pulltab[] = {
		TRA_INS_PUL, TRA_LOO_PUL, TRA_SEG_PUL, TRA_FLI_PUL, TRA_NEX_PUL, TRA_REP_PUL,
	};
	...
	set_pultab_disable(mh, cel_pulltab, Array_els(cel_pulltab), (thecel == NULL));
	set_pultab_disable(mh, anim_pulltab, Array_els(anim_pulltab), (flix.hdr.frame_count < 2));
	set_pultab_disable(mh, alt_pulltab, Array_els(alt_pulltab), (vl.alt_cel == NULL));
...
static int new_pull_list(Pull **ppull, Names *nlist, int ncount, int startid)
{
	...
	for (count = 0; count < ncount; count++) {
		...
		err = new_pull(&new, nlist->name);

		if (err < Success) {
			// early out; perform the cleanup below
			break;
		}
