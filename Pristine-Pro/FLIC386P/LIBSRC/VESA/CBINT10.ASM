...
;*
;* 10/24/93	Ian
;*		The allocation routine was still coded for the old-style
;*		logic wherein a flat-model pointer in edi was converted to
;*		a DOS segment by shifting and masking.	The new logic (in
;*		VESAINTF.ASM and WCRALLOC.ASM) requires that we interpret
;*		DI as a DOS segment without any special conversion.
;*****************************************************************************
...
int10_intercept proc near
	...
	mov	[ebp].STK_EDI,0 	; now realmode int10 will see 0 in DI.
#punt:
	pop	eax
...
;* _pj_vesa_int10
;*
;*  Issue INT10 call to VESA BIOS, such that DOS paragraph number in DI is
;*  seen by the real mode handler as ES:0.
;*  For codebuilder, we
